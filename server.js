...
app.get("/scrape", function(req, res) {
	request("https://www.deadspin.com/", function(error, response, html) {
		...
		var result = {};
		$("div.sc-3kpz0l-1").each(function(i, element) {
			var link = $(element).find("a.sc-1out364-0").attr("href");
			var title = $(element).find("h1.sc-759qgu-0").text().trim();
			var summary = $(element).find("p").text().trim();
			var img = $(element).parent().find("div.dv4r5q-3").find("img").attr("srcset");
			result.link = link;
			result.title = title;
			if (summary) {
